...
- 云同步。
- 助手回复正文的持久化采集（Claude Code 与 Codex 均已支持对话视图懒读展示，见下文 Web UI）。
- wrapper / PTY 终端采集。
...
- 对话视图：详情页按需读取整 session 对话（Claude Code 从 transcript、Codex 按 sessionId 定位 session 文件），渲染完整的 user/assistant/tool 时间线；复制对话时会先脱敏。
...
- raw hook payload 默认关闭。
- 助手回复按需现读展示（Claude Code 从 transcript、Codex 按 sessionId 定位 session 文件），不会被 prompt-capture 持久化；本地界面展示原文，复制对话文本时会先脱敏。
